...
class AmbassadriceCustoms
{
    ...
    /**
     * Récupère les identifiants des produits en promotion.
     *
     * Cette méthode interroge la base de données pour obtenir tous les produits
     * en promotion. Elle traite ensuite les données pour créer un tableau associatif
     *
     * @return array $list_promo_id Un tableau associatif contenant les identifiants des produits
     *                               en promotion et leur prix de vente.
     */
    ...
    /**
     * Récupère les données des commandes depuis l'API WooCommerce.
     *
     * Cette méthode interroge l'API WooCommerce pour obtenir les commandes
     * créées entre deux dates spécifiques. Les dates sont définies en fonction
     * du moment actuel, avec un intervalle de 2 jours dans le passé et dans le futur.
     * 
     * @return array $donnees Un tableau contenant les données des commandes récupérées
     *                        depuis l'API WooCommerce.
     */
    public function getdonneesall()
    {
        ...
        $currents2 = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $currents1)->format('Y-m-d\TH:i:s');
        $date_after = $currents2;
        // ajouter un intervale plus un jour !
        $current1 = $current->addDays(2);
        $current2 = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $current1)->format('Y-m-d\TH:i:s');
    ...
    /**
     * Récupération des commandes live et des codes élèves pour les ambassadrices.
     * qui incluent des codes de réduction associés aux ambassadrices. Elle traite les données
     * pour extraire les informations pertinentes et retourne un tableau de commandes uniques
     * liées aux codes des ambassadrices.
     *
     * @return array $array_data_uninque Un tableau associatif contenant les commandes filtrées pour l'activité des ambassadrices.
     */
    public function getDataorder(): array
    {
        
        $coupons = [];
        // recupérer les données du customer
        ...
        // recupérer la liste des cdoes speicifique dans un tableau
                ...
                foreach ($donnes['coupon_lines'] as $key => $val) {
                    foreach ($donnes['line_items'] as $ks => $vals) {
                        if (count($donnes['coupon_lines']) != 0) {
                            if (strpos($val['code'], $chaine) === true || strpos($val['code'], $chaine1) === true) {
                                if (strpos($val['code'], $chaine2) === false && strpos($val['code'], $chaine3) === false) {
                                    ...
                                    if ($donnes['payment_method'] == "payplug" || $donnes['payment_method'] == "oney_x4_with_fees") {
                                        $coupon_cards[] = $donnes;
                                    }
                                }
                            }
