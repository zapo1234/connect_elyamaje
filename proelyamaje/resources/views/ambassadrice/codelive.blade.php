			...
			<div class="page-content">
				...
				@if($message)
					<div class="d-flex w-100 justify-content-center">
						<div style="position: absolute;z-index: 1;top: 65px;" class="alert alert-success border-0 bg-success alert-dismissible fade show">
				...
				@elseif($message_error)
					<div class="d-flex w-100 justify-content-center">
						<div style="position: absolute;z-index: 1;top: 65px;" class="alert alert-danger border-0 bg-danger alert-dismissible fade show">
							<div class="text-white">{{ $message_error  }}</div>
							<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
						</div>
					</div>
				@endif
								...
								<tbody>
								  ...
								  @foreach($userlives as $resultat)
									<tr>
										...
										<td data-label="Nom"><span class="" id="nx{{  $resultat['id'] }}" style="font-size:16px;color:black;font-weight:200;"> {{ $resultat['username'] }} {{ $resultat['name'] }}</span></td>
										...
										<td data-label="Téléphone"><span class="mois" style="">  <i class="fas fa-phone"></i>  {{ $resultat['telephone'] }}</span></td>
										<td data-label="Status"> 
											<div class="h" style="font-size:13px;color:black;font-weight:600;">
												...
												@if($resultat['status'])
													...
													@if($resultat['css'] == "demandecode")
														...
														@endif	
													@elseif($resultat['css'] == "activecode" && $resultat['nombre_fois'] == 1) 
														<div class="badge rounded-pill text-success bg-light-success p-2 text-uppercase px-3"><i class="bx bxs-circle me-1"></i>Live en cours</div>
													@else 
