...
<head>
    ...
    #table_orders th{background-color:#eee;width:30%;height:50px;text-align:center;}
    #table_orders{width:90%;margin-left:7%;margin-top:-80px;}
    #table_orders td{width:30%;text-align:center;border:1px solid black;}
    #total{margin-left:64%;margin-top:40px;border:2px solid black;}
    .aei{display:none;}
...
<body>
    ...
    <div class="row">
        ...
        <div class="col-xl-8 col-lg-7">
            ...
            @foreach($data as $resultat)
             <tr>
              <td>{{ $resultat['datet'] }}</td>
               <td>{{ $resultat['id_commande']  }}</td>
               <td> {{ $resultat['customer'] }} </td>
              <td> {{ number_format($resultat['total_ht'], 2, ',', '') }}</td>
   ...
   <table class="table table-striped" id="table_orders" style="margin-top:10px">
    ...
    <table id="total" style="margin-top:20px">
     <tr>
       <td style="font-size:20px;color:black;font-weight:bold">{{ $ht }}</td>  
       <td style="font-size:20px;padding-left:2%;color:#000;"> {{ number_format($somme, 2, ',', '') }} € </td>
    </tr>
    
        <tr>
       <td style="font-size:20px;color:black;font-weight:bold">{{ $tva }}</td>  
       <td class="{{ $css1 }}" style="font-size:20px;color:black;font-weight:bold">{{ number_format($som, 2, ',', '') }} €</td>  
    </tr>
    
     </tr>
    
        <tr>
       <td style="font-size:20px;color:black;font-weight:bold">{{ $ttc }}</td>  
